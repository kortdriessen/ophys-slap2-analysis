classdef spineAnalysisPeaks < handle
    ...
    properties
        ...
        Clevel = 1000; %adjustable threshold for display of correlation image, in standard deviations
    end

    methods
        function obj = spineAnalysisPeaks(fn, selDMD)
            if nargin<1 || isempty(fn)
                [obj.fn, obj.dr] = uigetfile('*.mat', 'Select a summary file');
            else
                [obj.dr, obj.fn] = fileparts(fn);
            end
            ...
            IMavg = max(0,IMavg-prctile(IMavg(:),10));
            obj.IMavg = sqrt(IMavg./prctile(IMavg(:), 99));

            obj.IMact = obj.exptSummary.actIM{obj.selDMD};
            if size(obj.IMact,3)<2
                obj.IMact(:,:,2) = 0; %we don't do localization for channel 2
            end
            ...
            hold(obj.hAx, 'on');
            colors = spring(size(obj.exptSummary.peaks,2));
            for trialIx = 1:size(obj.exptSummary.peaks,2)
                scatter(obj.exptSummary.peaks{selDMD,trialIx}.col, obj.exptSummary.peaks{selDMD,trialIx}.row, 5*obj.exptSummary.peaks{selDMD,trialIx}.val.^2, 'MarkerEdgeColor',colors(trialIx,:), 'parent', obj.hAx);
            end
        end
