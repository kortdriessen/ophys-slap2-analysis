...
for fnIx = 1:length(fns)
    fn1 = fns{fnIx};
    IM = double(networkScanImageTiffReader([dr filesep fn1]));

    nameIx = strfind(fn1, '_REGIS');
    aDataDir = dir([dr filesep fn1(1:nameIx) '*ALIGNMENTDATA.mat'])
        if isempty(aDataDir)
        error("Couldn't find alignment data!");
    end
    aDataFn = aDataDir(1).name;
    load([dr filesep aDataFn], 'aData');
    nChan = aData.numChannels;
    frameRate = aData.alignHz;
    ...
    for ch = [1]
        ...
        if ch==1
            ...
            act = act./somaIM;
            dFmax = 1.2*prctile(act(~nans), 99.99);%what percentile of activity pixels gets saturated
            act = act./dFmax;
