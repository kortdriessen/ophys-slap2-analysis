...
p = Processing.create_with_sequential_process_graph(
    ...
    data_processes=[
        ...
        DataProcess(
            ...
            stage=ProcessStage.PROCESSING,
            start_date_time=datetime.fromisoformat(extraction_metadata.get('startTime', datetime.now(timezone.utc).astimezone().isoformat())),
            end_date_time=datetime.fromisoformat(extraction_metadata.get('endTime', datetime.now(timezone.utc).astimezone().isoformat())),
            output_path="",
            pipeline_name="SLAP2 multi-ROI raster processing pipeline (Matlab)",
            code=example_code.model_copy(
                update=dict(
