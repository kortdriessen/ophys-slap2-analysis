function harpData = read_harp_bin(filePath, loadcell)
%{
 21 byte width
loadCellData.' =
...
if data(1) == 3
    ...
    disp('Warning: You are not reading HARP events')
end
...
if useTimeStamps == true
    ...
    payloadData = data(:, 6:(6+1+payloadSize)); %ignoring headers
    %Seconds are 4 bytes
    ...
    dataValuesSlice = payloadData(:, (secondsPayload + ticksPayload+1 ):payloadSize);
    ...
    dataValuesSlice = payloadData(:, 1:payloadSize);
    dataValues =  arrayfun(@(i) typecast(dataValuesSlice(i,:), payloadType), 1:size(payloadData,1), 'UniformOutput', false);
    pinsRead = numel(dataValues{1});
    dataValues = reshape(cell2mat(dataValues), 5, []).';
