...
for trialIx = length(fns):-1:1
    ...
    meanIM(:, end:size(IM,2),:,:) = nan;%make nan image
    meanIM(:,:,:,trialIx) = nan;%for each trial
    meanIM(1:size(IM,1),1:size(IM,2),:,trialIx) = mean(IM,4, 'omitnan');%average intensity mean projection for given trial
