...
for f_ix = 1:length(fns)
    ...
    options_rigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'bin_width',200,'max_shift',15,'us_fac',4,'init_batch',200, 'correct_bidir', false);
    ...
    saveastiff(M2, [dr filesep fn(1:end-4) '_DSaligned.tif'])
    catch
