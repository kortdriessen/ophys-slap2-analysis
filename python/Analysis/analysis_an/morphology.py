...
def _to_newick(morph_tree, root_name = 'ROOT'):
    ...
    level = [_to_newick(mtree, root_name = _rename_braces(name[0])) for name, mtree in tree_dict.iteritems()]
        ...
        for m_idx, m in marking_df.iterrows():
            ...
            if legend_text and (m_idx, legend_text) in unique_markings:
                ...
                annotation.append({
                    ...
                    "annotation_text": m.annotation_text if 'annotation_text' in m and not pd.isnull(m.annotation_text) else ""
                })
...
def plot_phylogram(hoc_fpath, annotation_df = None, ax = None, figsize = (10,10), lw = 0.35, trunk_lw = 0.7,
    ...
    if plotter == 'pyplot': 
        ...
        # add scalebar
        scalebar = an_plots.AnchoredHScaleBar(size = 100, label = "100 $\mu$m", loc = 4, frameon = False, 
        ...
        if annotation_df is not None and 'legend_text' in annotation_df and not annotation_df['legend_text'].isnull().all():
            ...
            handles, labels = an_plots.sort_legend_labels(ax)
